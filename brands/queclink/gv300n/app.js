...
const Gv300n = require('./parser/gv300n');
...
(async () => {
  ...
  const processAndSaveGPSData = (gpsdata, socket_info) => {
    const gps = new Gv300n(gpsdata, socket_info);
  ...
  if (protocol === 'udp') {
    ...
    console.log(`Escuchando paquetes UDP en el puerto ${port} ip ${process.env.LOCANET_IP} para el modelo ${model} de la marca ${brand}`);
