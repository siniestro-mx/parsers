  ...
  */
  ...
  constructor(data, socket_info) {
    ...
    */
    ...
    this.model = Gv300n.#getModel(data[1]).toLowerCase();
    ...
    this.msg_type = Gv300n.#getMsgType(data[0]);
    ...
    this.firmware = Gv300n.#getFirmwareVersion(data[1]);
    this.event = Gv300n.#getEvent(data[0]);
    ...
    try {
      const parsed_data = Gv300n.#parse(data, socket_info, this);
    ...
    if (!this.data.valid) {
      this.error = 'Error en paquete Gv300n';
      return;
    }
    ...
    if (!this.data.SendTime && !["GTALC", "GTCID", "GTALM", "GTALL"].includes(this.event)) {
      if (!this.data.GPSUTCTime) {
        this.error = 'Paquete sin SendTime de Gv300n';
        return;
      }
      else {
  ...
  static #parseReport(data, socket_info, me) {
    ...
    if (event === "GTERI") {
      try {
        ({ index, data } = Gv300n.#specialCases(data, index, me));
      } catch (e) {
  ...
  static #parse(data, socket_info, me) {
    ...
    switch (msg_type) {
      case "acknowledgment":
        return Gv300n.#parseAcknowledgement(data, socket_info, me);
      case "report":
      case "buffer":
      case "config":
        return Gv300n.#parseReport(data, socket_info, me);
    }
  }
}

module.exports = Gv300n;